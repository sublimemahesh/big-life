    ...
    @for ($i = 1; $i <= config('genealogy.children', 2); $i++)
        ...
        <li class="position-{{ $i }}">
            ...
            @if (isset($descendant))
                ...
                @endphp
                <a href="{{ $level === 3 ? route('user.genealogy', $descendant) : 'javascript:void(0)' }}" @class(["next-genealogy" => $level === 3])>
                    @include('backend.user.genealogy.includes.genealogy-card', ['user' => $descendant])
                </a>
                @if($level <= 2)
                    @include('backend.user.genealogy.includes.genealogy-tree-ul', ['descendants' => $descendant->children ,'level' => $level])
                @endif
            @else
