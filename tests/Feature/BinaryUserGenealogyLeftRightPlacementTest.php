...
class BinaryUserGenealogyLeftRightPlacementTest extends TestCase
{
    // use RefreshDatabase;
    ...
    public function it_places_user_in_left_side_under_one_user()
    {
        ...
        $this->assertEquals($superParent->id, $purchasedUser->parent_id);
    ...
    public function it_places_user_left_under_parent_with_one_child()
    {
        ...
        NewUserGenealogyAutoPlacement::dispatch($purchasedUser)->onConnection('sync');
        $purchasedUser->refresh();
        $this->assertEquals($superParent->id, $purchasedUser->super_parent_id);
    ...
    /** @test */
    public function it_places_user_left_under_parent_user1()
    {
        $superParent = User::find(3);
        $parent = User::find(6);
        $purchasedUser = User::factory()->create(['id' => 7, 'super_parent_id' => $superParent->id, 'parent_id' => null, 'position' => BinaryPlaceEnum::LEFT->value]);
